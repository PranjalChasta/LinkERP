...
import { RoleService } from 'src/app/sys/services/role.service';
import { SysCommonService } from 'src/app/sys/services/sys-common.service';

@Component({
  selector: 'app-role-detailas',
  templateUrl: './role-detailas.component.html',
  styleUrls: ['./role-detailas.component.css'],
  providers: [{
    provide: NG_VALUE_ACCESSOR,
    useExisting: forwardRef(() => RoleDetailasComponent),
    multi: true
  }]
})
export class RoleDetailasComponent implements OnInit {
  ...
  Value: any;
  constructor(private sysCommonService: SysCommonService, private Roleservice: RoleService,) { }
  ...
  config = {
    ...
    searchPlaceholder: 'Search Role', // label thats displayed in search input,
  ...
  BindRoles() {
    ...
    this.Roleservice.getAllRoles().subscribe((resp: any) => {
      let Roles: any[] =  resp.data.roles;
      Roles.forEach(p => {
        let option = {
          id: p.id,
          code: p.roleCode,
          description:p.roleName + ' - ' + p.roleCode
        }
        options.push(option);
