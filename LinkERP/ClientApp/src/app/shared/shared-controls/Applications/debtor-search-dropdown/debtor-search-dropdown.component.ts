...
@Component({
  selector: 'app-debtor-search-dropdown',
  templateUrl: './debtor-search-dropdown.component.html',
  styleUrls: ['./debtor-search-dropdown.component.css'],
  providers: [{
    provide: NG_VALUE_ACCESSOR,
    useExisting: forwardRef(() => DebtorSearchDropdownComponent),
    multi: true
  }]
})
export class DebtorSearchDropdownComponent implements OnInit, ControlValueAccessor {
  @Input()
  RetrunValue: any = 'ID';
  ...
  config = {
    ...
    searchPlaceholder: 'Search Customer', // label thats displayed in search input,
  ...
  BindDebtors() {
    let options: any[] = [];
    this.sysCommonService.getDebtors().subscribe((resp: any) => {
      let Debtor: any[] = resp.data.debtors;
      Debtor.forEach(w => {
        let option = {
          id: w.id,
          code: w.debtorCode,
          description: w.debtorCode + ' - ' + w.debtorAccountName
        }
        options.push(option);
