...
@Component({
  selector: 'app-currency-search-dropdown',
  templateUrl: './currency-search-dropdown.component.html',
  styleUrls: ['./currency-search-dropdown.component.css'],
  providers: [{
    provide: NG_VALUE_ACCESSOR,
    useExisting: forwardRef(() => CurrencySearchDropdownComponent),
    multi: true
  }]
})
export class CurrencySearchDropdownComponent implements OnInit {
  @Input()
  RetrunValue: any = 'ID';
  ...
  config = {
    ...
    placeholder: '--Select Cashier--',// text to be displayed when no item is selected defaults to Select,
    ...
    searchPlaceholder: 'Search Cashier', // label thats displayed in search input,
  ...
  BindCashier() {
    ...
    this.sysCommonService.getCurrency().subscribe((resp: any) => {
      debugger;
      let Currency: any[] = resp.data.cuurency;
      Currency.forEach(w => {
        let option = {
          id: w.id,
          code: w.currencyCode,
          description: w.currencyCode + ' - ' + w.currecnyName
        }
        options.push(option);
