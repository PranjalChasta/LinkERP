...
@Component({
  selector: 'app-adjustment-dropdown',
  templateUrl: './adjustment-dropdown.component.html',
  styleUrls: ['./adjustment-dropdown.component.css'],
  providers: [{
    provide: NG_VALUE_ACCESSOR,
    useExisting: forwardRef(() => AdjustmentDropdownComponent),
    multi: true
  }]
})
export class AdjustmentDropdownComponent implements OnInit, ControlValueAccessor {
  @Input()
  RetrunValue: any = 'ID';
  ...
  config = {
    ...
    placeholder: '--Select Adjustment --',// text to be displayed when no item is selected defaults to Select,
    ...
    searchPlaceholder: 'Search Adjustment', // label thats displayed in search input,
  ...
  BindAdjustments() {
    let options: any[] = [];
    this.sysCommonService.getAdjustments().subscribe((resp: any) => {
      let Adjustment: any[] = resp.data.adjustment;
      Adjustment.forEach(w => {
        let option = {
          id: w.id,
          code: w.adjustmentNo,
          description: w.adjustmentNo 
        }
        options.push(option);
